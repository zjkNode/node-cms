<template>
    <div class="act_edit">
        <h2>活动 — 新增</h2>
        <el-row>
            <el-form label-width="90px" :model="formData" ref="formData" :rules="rules" :show-message="true">
                ...
                <el-form-item label="设置背景">
                    ...
                    <el-col :span="4" style="padding-left:10px;" v-show="formData.data.style['background-image']!=''">
                ...
                <el-form-item label="模板内容" prop="data">
                    <el-row class="patsCol" id="patsCol">
                        <template v-for="(part, index ) in parts" v-if="allParts[part].type !== 'hidden'">
                            <div v-bind:is="part" :ref="part+'_'+index"></div> 
                        </template>


                        <template v-for="(part, index ) in parts">
                            <div v-bind:is="part" :ref="part+'_'+index" v-if="allParts[part].type === 'hidden'"></div>
                        </template>
                    </el-row>
...
<script>
    ...
    export default{
        ...
        methods:{
            ...
            getTplData(){
                let tplid = Util.getQueryParams("tplid"),
                    url = '/api/act/tpl/'+tplid;
                this.$http.get(url).then((rs)=>{
                    if(rs.body.code != 'SUCCESS'){
                        return;
                    }
                    let resData = rs.body.data;
                    this.formData.parts = resData.parts;
                    this.formData.timeId = resData.timeId;
                    this.formData.activityCode = resData.activityCode;
                    this.formData.data.activityCode = this.formData.activityCode;
            ...
            addPartsData(){
                ...
                for(var i = 0; i< this.parts.length; i++){
                    ...
                    let part_index = part+'_'+i,
                        part_ref = this.$refs[part_index][0] || this.$refs[part_index],
                        part_data = part_ref.getData();
                    this.formData.data.components.push ( part_data );
                    part_data.image_path && part_data.image_path.length == 0 ? this.tplContent = false : this.tplContent = true;
                };
            },
            ...
            handleWrapConfig(){
                ...
                $wrap.css({
                    ...
                    'background-repeat': this.formData.data.style['background-repeat'] || '',
                });
            },
            leaveForm(next){
